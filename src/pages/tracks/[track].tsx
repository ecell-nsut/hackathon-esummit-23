...
const TrackInfo: FC<TrackInfoProps>=(props) => {
  return (
    <div className='flex items-center justify-center p-4 mt-20 h-full'>
      <div className={`flex flex-col items-center md:items-start justify-center max-w-screen-xl space-y-4`}>
        <h1 className={`text-4xl sm:text-5xl md:text-6xl font-extrabold tracking-tight text-center text-white md:text-left mb-8`}>
          {props.title}
        </h1>
        <p className={`text-lg sm:text-xl md:text-2xl text-justify text-gray-200 mb-12 max-w-screen-md`}>
          {props.description}
        </p>
        ...
        </div>
        {
          ...
          <div className="pb-3 pt-2 text-left text-sm text-gray-600 dark:text-gray-300 md:pb-6 md:pt-3 max-w-xl">
            <span>
              Sponsored tracks have <p className="text-blue-500 inline">special prizes</p> from respective sponsor.
              The winner in sponsored track is subject to approval from respective sponsor.
            </span>
          </div>
        }
      </div>
